import Layout from "@templates/Layout";

const Home = () => (
  <Layout>
    <section className="smesh">
      <div className="aureole two">
        <div className="one valign">
          <div className="centertxt pad">
            <h1>
              Pablo <br />
              Orozco Montes
            </h1>
            <p>Publicista, Escritor, Artista, Programador. </p>
          </div>
        </div>
        <div className="two">
          <div className="block-img">
            <img src="https://images.prismic.io/pablopvsky/5eb03355-0b13-4595-b58d-fead1e0eafa0_pablo+orozco+montes+pablopvsky.jpg?auto=compress,format" />
          </div>
        </div>
      </div>
    </section>
    <section className="pad">
      <div className="smesh">
        <div className="centertxt">
          <div className="pad">
            <h2>El Mundo no es tan teso</h2>
            <p className="smash">
              Todas las cosas están interconectadas entre sí, por lo tanto,
              podemos eliminar cualquier barrera existente entre cualquier tema.
            </p>
          </div>
          <div className="block-img smosh figure-down">
            <img src="https://images.prismic.io/pablopvsky/5b66ab04-19c3-4755-a103-ec8bd96e9052_figura+pablopvsky.png?auto=compress,format" />
          </div>
        </div>
      </div>
    </section>
    <section className="pad blue">
      <div className="smash">
        <div className="pad centertxt">
          <p>
            Todas las cosas están interconectadas entre sí, por lo tanto,
            podemos eliminar cualquier barrera existente entre cualquier tema.
          </p>
        </div>
        <div className="centertxt">
          <h3>Últimas publicaciones</h3>
        </div>
      </div>
      <div className="smesh">
        <div className="aureole three">
          <div className="one">
            <div className="mod">
              <div className="mod-media zoom">
                <img src="https://images.prismic.io/pablopvsky/cedc926f-b893-4194-b2a0-5056388c52e5_garitma_6c1c4bbe-430e-457a-ae2b-fe45ba615466_featured-pablopvsky.jpg?auto=compress,format" />
              </div>
              <div className="mod-detail">
                <span className="mod-title">
                  iA Writer la aplicación de escritura minimalista que mejorará
                  tu productividad
                </span>
                <p>
                  Un entorno limpio, simple y ágil a la hora de escribir que
                  convierte a iA Writer en una de las aplicaciones de escritura
                  más populares de iOS, Mac Os, Android y Windows.
                </p>
              </div>
            </div>
          </div>
          <div className="two">
            <div className="mod">
              <div className="mod-media zoom">
                <img src="https://images.prismic.io/pablopvsky/cedc926f-b893-4194-b2a0-5056388c52e5_garitma_6c1c4bbe-430e-457a-ae2b-fe45ba615466_featured-pablopvsky.jpg?auto=compress,format" />
              </div>
              <div className="mod-detail">
                <span className="mod-title">
                  iA Writer la aplicación de escritura minimalista que mejorará
                  tu productividad
                </span>
                <p>
                  Un entorno limpio, simple y ágil a la hora de escribir que
                  convierte a iA Writer en una de las aplicaciones de escritura
                  más populares de iOS, Mac Os, Android y Windows.
                </p>
              </div>
            </div>
          </div>
          <div className="three">
            <div className="mod">
              <div className="mod-media zoom">
                <img src="https://images.prismic.io/pablopvsky/cedc926f-b893-4194-b2a0-5056388c52e5_garitma_6c1c4bbe-430e-457a-ae2b-fe45ba615466_featured-pablopvsky.jpg?auto=compress,format" />
              </div>
              <div className="mod-detail">
                <span className="mod-title">
                  iA Writer la aplicación de escritura minimalista que mejorará
                  tu productividad
                </span>
                <p>
                  Un entorno limpio, simple y ágil a la hora de escribir que
                  convierte a iA Writer en una de las aplicaciones de escritura
                  más populares de iOS, Mac Os, Android y Windows.
                </p>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div className="pad smash centertxt">
        <a className="button-link">Ver más</a>
      </div>
    </section>
    <section>
      <div className="smush">
        <div className="block-img">
          <img src="https://images.prismic.io/pablopvsky/9d60f9b0-b3fc-4332-bc2f-95df7afeb9d4_arrow-down.png?auto=compress,format" />
        </div>
        <div className="pad centertxt smash">
          <h3>Proyectos</h3>
          <p>
            At vero eos et accusamus et iusto odio dignissimos ducimus qui
            blanditiis praesentium voluptatum deleniti atque corrupti quos
            dolores.
          </p>
        </div>
      </div>
      <div className="smesh">
        <div className="aureole">
          <div className="one block-img">
            <img src="https://images.prismic.io/pablopvsky/985fe68a-6f12-45ee-bbb2-9fedc46b28b9_proyecto.png?auto=compress,format" />
          </div>
          <div className="two block-img">
            <img src="https://images.prismic.io/pablopvsky/3c9d1d11-e327-4ca2-bad6-bd31b99830b5_proyecto3.png?auto=compress,format" />
          </div>
          <div className="three block-img">
            <img src="https://images.prismic.io/pablopvsky/81aad21f-f0e6-4e4f-8806-d053cbdad1ce_proyecto2.png?auto=compress,format" />
          </div>
        </div>
      </div>
      <div className="smash pad">
        <div className="block-img">
          <img src="https://images.prismic.io/pablopvsky/85992e77-9d0e-4fcf-8f54-f86b2e5ac99f_keyvisual+pablopvsky.jpg?auto=compress,format" />
        </div>
      </div>
    </section>
    <section className="smush pad">
      <div className="block-img">
        <img src="https://images.prismic.io/pablopvsky/9d60f9b0-b3fc-4332-bc2f-95df7afeb9d4_arrow-down.png?auto=compress,format" />
      </div>
      <div className="smash centertxt">
        <h3>Lorem ipsum</h3>
        <p>
          At vero eos et accusamus et iusto odio dignissimos ducimus qui
          blanditiis praesentium voluptatum deleniti atque corrupti quos
          dolores.
        </p>
      </div>
      <div className="pad">
        <div className="block-img">
          <img src="https://images.prismic.io/pablopvsky/cd532d9c-738b-4cf8-b10e-cbd0f0ff5dc3_figura+pablopvsky+2.png?auto=compress,format" />
        </div>
      </div>
    </section>
    <section className="smash">
      <div className="centertxt">
        <h3 className="h1 ">Hagamos algo interesante juntos</h3>
        <p>
          At vero eos et accusamus et iusto odio dignissimos ducimus qui
          blanditiis praesentium voluptatum deleniti atque corrupti quos
          dolores.
        </p>
      </div>
      <div className="smosh pad">
        <div className="inputer">
          <input />
          <input />
          <input />
          <button className="button-fill fluid">
            <span className="container">Enviar</span>
          </button>
        </div>
      </div>
    </section>
  </Layout>
);

export default Home;
